...
write_VTK_unstructured_Points(inputPars *par, struct grid *g){
  ...
  if (verbose) fprintf(stderr, "writevtkstart %.1e %.1e %.1e allocated: dens %.3e, temp %.3e, abun %.3e\n", g[500].x[0], g[500].x[1], g[500].x[2], g[500].dens[0], g[500].t[0], g[500].abun[0]);
  ...
  if (verbose) fprintf(stderr, "writevtkend %.1e %.1e %.1e allocated: dens %.3e, temp %.3e, abun %.3e\n", g[500].x[0], g[500].x[1], g[500].x[2], g[500].dens[0], g[500].t[0], g[500].abun[0]);
...
buildGrid(inputPars *par, struct grid *g){
	...
	for(k=0;k<par->pIntensity;k++){ //customised gridding setup
	  temp=gsl_rng_uniform(ran);
	  flag=0;
	  /* Pick a point and check if we like it or not */
	  if (k<par->pIntensity/20) do{
	    ...
	    }while(!flag); //first 5% of points are sampled in uniform cartesian space regardless, helps to sample outer regions
	  
	  else {
	    do{
	      if(par->sampling==0){ //log rad sampling
		...
		if(DIM==3) z=r*cos(phi);
	      ...
	      else if(par->sampling==1){ //cartesian sampling
		x=(2*gsl_rng_uniform(ran)-1)*par->radius;
		y=(2*gsl_rng_uniform(ran)-1)*par->radius;
		if(DIM==3) z=(2*gsl_rng_uniform(ran)-1)*par->radius;
		else z=0;
	      } 
	      else if(par->sampling==2){ //disc sampling
		if (k > 2*par->pIntensity/10) r=pow(10,logmin+gsl_rng_uniform(ran)*(lograd-logmin));
		else {
		...
		x=r*cos(theta);//*sin(phi);
		y=r*sin(theta);//*sin(phi);
		if(DIM==3) z=(2*gsl_rng_uniform(ran)-1)*pow(gsl_rng_uniform(ran)*sqrt(par->radius),2);//sqrt z sampling  //z=r*cos(phi);
		else z=0.;
	      }
	      else if(par->sampling==3){  //disc sampling w/star
		if (k > 98*par->pIntensity/100){
		  r=0.01*AU*(0.8+0.25*gsl_rng_uniform(ran));
		  if (verbose) fprintf(stderr, "%d %.3e\n",k,r);
		}
		...
		x=r*cos(theta);//*sin(phi);
		y=r*sin(theta);//*sin(phi);
		if(DIM==3) z=(2*gsl_rng_uniform(ran)-1)*par->radius;//z=r*cos(phi);
		else z=0.;
	      }
	      else if(par->sampling==4){ // cylindrical, uniform in z,phi, log r
		r=pow(10,logmin+gsl_rng_uniform(ran)*(lograd-logmin));
		phi=2.*PI*gsl_rng_uniform(ran);
		
		z=(2*gsl_rng_uniform(ran)-1)*par->radius;	
		x=r*cos(phi);
		y=r*sin(phi);
	      }
	      else if(par->sampling==5){ //disc sampling, centre axis along x
		if (k > 2*par->pIntensity/10) r=pow(10,logmin+gsl_rng_uniform(ran)*(lograd-logmin));
		else {
		  r=pow(10,(logmin+lograd)/2+gsl_rng_uniform(ran)*(lograd-logmin)/2);
		  par->minScale=30*AU;
		...
		y=r*sin(theta);//*sin(phi);
		if(DIM==3) x=(2*gsl_rng_uniform(ran)-1)*par->radius;//x=r*cos(phi);
		else z=0.;
	      }
	      else {
	...
	/* for(k=0;k<par->pIntensity;k++){ */ //default gridding setup
	/* 	temp=gsl_rng_uniform(ran); */
	/* 	flag=0; */
	/* 	/\* Pick a point and check if we like it or not *\/ */
	/* 	do{ */
	...
	for(i=0;i<par->pIntensity;i++){
		...
		if (verbose) fprintf(stderr, "gridpoint %.1e %.1e %.1e allocated: dens %.3e, temp %.3e, abun %.3e\n", g[i].x[0], g[i].x[1], g[i].x[2], g[i].dens[0], g[i].t[0], g[i].abun[0]);
	}
...
//	getMass(par,g, ran);
	...
	dumpGrid(par,g);
	if (verbose) fprintf(stderr, "final %.1e %.1e %.1e allocated: dens %.3e, temp %.3e, abun %.3e\n", g[par->pIntensity/2].x[0], g[par->pIntensity/2].x[1], g[par->pIntensity/2].x[2], g[par->pIntensity/2].dens[0], g[par->pIntensity/2].t[0], g[par->pIntensity/2].abun[0]);
    gsl_rng_free(ran);
    free(abun);
	if(!silent) done(5);

	if (verbose) fprintf(stderr, "final2 %.1e %.1e %.1e allocated: dens %.3e, temp %.3e, abun %.3e\n", g[par->pIntensity/2].x[0], g[par->pIntensity/2].x[1], g[par->pIntensity/2].x[2], g[par->pIntensity/2].dens[0], g[par->pIntensity/2].t[0], g[par->pIntensity/2].abun[0]);
}
