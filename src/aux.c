...
levelPops(molData *m, inputPars *par, struct grid *g){
  int id,conv=0,iter,ilev,prog=0,ispec,c=0, flag=0;
  double percent=0.,pstate,*median,result1=0,result2=0, result3=0, result4=0;
  blend *matrix;
  ...
  if(par->lte_only==0){
    do{	
      ...
      if(!silent) progressbar2(prog++, 0, result1, result2,result3, result4);
      pstate=0.;
      ...
      gsl_sort(median, 1, c);
      if(conv>1){
        result1=median[c/100];
        result2=median[c/4];
	result3=gsl_stats_median_from_sorted_data(median, 1, c);
	result4=median[3*c/4];
      }
      free(median);
      
      if(!silent) progressbar2(prog, percent, result1, result2, result3, result4);		
      if(par->outputfile) popsout(par,g,m);
      if(result2>converged_1stquart && result1> converged_1stpercent) conv=NITERATIONS;
