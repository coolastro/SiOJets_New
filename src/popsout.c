...
popsout(inputPars *par, struct grid *g, molData *m){
	...
	fprintf(fp,"# Column definition: x,y,z,   H2,temp,abun,   vx,vy,vz,    ,pops_0...pops_n\n"); 
	for(j=0;j<par->ncell-par->sinkPoints;j++){
	    dens=0.;
	 	for(l=0;l<par->collPart;l++) dens+=g[j].dens[l];
		fprintf(fp,"%.5e %.5e %.5e   %.5e %.5e %.5e   %.5e %.5e %.5e    ", g[j].x[0], g[j].x[1], g[j].x[2],   dens, g[j].t[0], g[j].nmol[0]/dens,   g[j].vel[0], g[j].vel[1], g[j].vel[2]);
