 ...
 */
  ...
  img[0].velres= 750;
  img[0].pxls			= 256;	      // Pixels per dimension
  img[0].imgres			= 2;		  // Resolution in arc seconds
...
density(double x, double y, double z, double *density){	
  ...
  if ((R>(arr_size_R-1)) || (Z>(arr_size_Z-1)) || (R<0) || (Z<0)) {density[0]=rho[arr_size_R-1][arr_size_Z-1]*unit_density*(1-fHII[arr_size_R-1][arr_size_Z-1]);}
