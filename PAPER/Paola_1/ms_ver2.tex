...
Tafalla:2011p14051}. Advances in millimeter interferometery 
...
velocity of $v_{\rm jet,0}$ = 100 km/s and set the jet density to
$10^{5} \rm cm^{-3}$ within a radius of $2.5\times10^{15} \rm
...
or internal shocks inside the jet \cite{Bachiller:1996p4692}.
...
equilibrium for all our runs. 
Based on these profiles (adopted from \citealt{Stone:2000p2650}), 
...
\cite{Hartigan:2007p15768} has shown that at large distances of the 
...
value of 10$^{-5}$ as measured in dense cores like L1554 \refer. 
...
these are the step profile, where the abundance of SiO is 10$^{-12}$ below 20 km$\,$s$^{-1}$ and 10$^{-6}$ at greater velocities, and the top hat profile, in which the SiO abundance is the same as the step profile but returns to 10$^{-12}$ at velocities greater than 100 km$\,$s$^{-1}$.
In order to get rid of discontinuous changes in abundance, we also
...
10-20\% measured in optical jets \cite{Bacciotti:1995p15970}.
...
Radiative transfer modelling using a homogenous slab of material and large velocity gradient (LVG) photon escape probability have been unable to reproduce these line ratios. 
In the 3D non-LTE (non local thermodynamic equilibrium) radiative transfer modeling presented in this work, no assumption of optical depths or photon escape probability are prescribed and line ratios are
self consistently calculated. The line ratios are shown in the bottom left panel of
fig.~\ref{fig:lineratio}. Their values lie very close to unity, in agreement with observations. 
...
reaching up to 100 km\,s$^{-1}$. We have used four
...
as this could require including dust grains and sputtering 
