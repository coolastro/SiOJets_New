...
simultaneously study the dynamics along with complex molecular chemistry.}
  % methods heading (mandatory)
   { For such a study, we have perfomed MHD simulations
...
perpendicular to the underlying accertion disk plays a vital role in
...
driven molecular outflows not only fail to match observed PV digarams \cite{Cabrit:1992p14098}
but also tend to sweep large quantitity of material at the extermities
of the lobes \cite{Masson:1992p14101}. While the jet driven models could sucessfully
...
account for shock chemsitry. Instead, shock chemistry is studied
...
molecules surving in steady state disk winds have also been shown
...
%
In the present work, our goal is to take a step further in modelling of
...
tranistions, by combining axisymmetric MHD simulations of
radiative jet propogation. In particular, we evolve the jet dynamical
...
presecriptions and radiative transfer line code respectively. In
...
The amount of SiO is determined by the local velocity and
temperature. The fractional abundance is given by the equation:
...
In addition to this if the temperature at the point is greater than 92,000K (the temperature of the Si-O bond disassociation energy) the abundance is reduced to 10$^{-12}$.

\begin{figure}
 \includegraphics[width=84mm]{\figpath/diffabun.png}
 \caption{The different fractional SiO abundances as a function of
   shock velocity, V${\rm s}$.}
 \label{abun}
\end{figure}
...
prescribtion of cooling and the density contrast between the jet and
the ambient medium denoted by $\eta$. The various cooling prescribtions used for the present study are
described in details in section~\ref{sec:chem}. A value of $\eta > 1$, mean that
that jet is overdense with respect to the medium. In all our runs with
different cooling prescribtions, we have assumed the jet to be
overdense by 10 times that of the ambient medium. Additionally, for
...
The magentic field strength is kept to be fixed using $\beta = 10.0$,
...
The post-processing phase is done in details for the molecular run
with $\eta = 3$. To obtain the SiO emission maps and corresponding
...
describes all the profiles used for the present study. The obivious
...
abundance profiles are described in table~\ref{tab:result2} 
...
\hline
Profile & v$_{\rm j}$ [km s$^{-1}$] & $\delta^{-1}$ = v$_{\rm j}$/v$_{\rm s}$ & Peak Intensity [K] & $\Delta$v [km s$^{-1}$]\\ 
\hline\hline
Top Hat & 100.0 & 1.0 & & \\
Functional & 100.0 & 1.0 & & \\
Functional & 100.0 & $(1.0 + \eta^{-0.5})$ &
& \\
Functional & 100.0 & $(1.0 + \eta(z)^{-0.5})$ &
& \\
Functional & 125.0 & $(1.0 + \eta(z)^{-0.5})$ &
& \\
Functional & 150.0 & $(1.0 + \eta(z)^{-0.5})$ &
& \\
\hline
\end{tabular}
\label{tab:result2}
...
figure nos. 2 -- Similuated images, emission maps for eta = 10.
\begin{figure*}
...
In case of molecular outflows, the most appropiate form of cooling
prescribtion among the ones described in sect.~\ref{sec:chem} is
...
$\eta$ = 3 and $\beta$ = 10. The jet is largely dominated by atmoic
...
jet. Ionzied hydrogen is mainly formed at the tip
...
highest tempeature of $\sim$ 50000\,K is attained in our flow at the
...
molecular hydrogen, regions with lowest values of $\mu$ are where most
of the ionized hydrogen is present, while along the jet the value
...
(3) intermediate knots. As the atomic jet propgates from the lower boundary 
...
figure nos. 2 -- Comparison of three profiles for SiO Abundance,
Contour maps for two transitions for top hat profile.
Effects of varing abundance profile ... Crucial ingredient in
modeling molecular bullets \\
\begin{figure}
 \includegraphics[width=1.\columnwidth]{\figpath/diffabunEM.png}
...
\begin{figure}
 \includegraphics[width=1.\columnwidth]{\figpath/PV_diffprof.png}
...
\subsection{Spectra and PV diagrams}
figure nos. 1 -- one transition 3 different angles, Comparison of
spectra and line ratios  
Spectra and PV Digrams\\
Line - withs $\sim$ 15-25 km\,s$^{-1}$. Wedge shaped PV diagrams
a signature feature of jet driven molecular outflows. Counterparts in
observations.
...
\begin{figure}
 ...
 \caption{Emission map ({\it top left}), PV diagram ({\it top right}
   ...
   $\eta$ = 3 and fractional abunance of functional form is used
